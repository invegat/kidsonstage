...
class EventDetailGroupRow extends Component {
  constructor(props) {
    ...
    const readOnly =
      (thisGroup !== undefined && thisGroup.name && thisGroup.name.length > 1) ||
      this.props.admin === 0;
    ...
    this.state = {
      ...
      loaded: !(this.props.eventId > 0 && (thisGroup !== undefined ? thisGroup.id : -1) > 0),
  ...
  toCurrentTime() {
    if (
      ...
      this.state.group.time.length > 0 &&
      this.state.group.time[0] === ' '
    ) {
  ...
  sendGroup(group, edit) {
    if (group.name && group.time && group.time !== '00:00' && group.time.length >= 5) {
      console.log(
        `group.name: ${group.name} group.time: ${group.time} group.eventId: ${group.eventId}`
      );
      this.props.isValid(
        this.props.index,
        group.id > 0 &&
  ...
  // }
  render() {
    const { fields, groupText, index, remove, removePart, edit } = this.props;
      ...
      <div onKeyPress={tab}>
        ...
        <Field
          ...
          onBlur={event => {
            ...
            if (
              Number(currentTime.slice(0, 2)) !== hour ||
...
export default connect(
  ...
  dispatch => ({
    ...
    removePart: partGroup => dispatch(deletePartGroup(partGroup)),
